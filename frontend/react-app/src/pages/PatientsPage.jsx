...
const EMPTY_FORM = {
  patientCode:"", age:"", gender:"MALE",
  disease:"", region:"", street:"",
  symptoms:"", diagnosis:"",
};
...
export default function PatientsPage({ user }) {
  ...
  useEffect(() => {
    ...
    locationAPI.getRegions().then(r => {
      if (Array.isArray(r)) setLocations(r);
    });
  }, []);
  ...
  const handleSubmit = async () => {
    ...
    try {
      let result;
      if (editPatient) {
        result = await patientAPI.update(editPatient.id, form);
      } else {
        result = await patientAPI.create({ ...form, age: parseInt(form.age) });
      }
      if (result) {
  ...
  const handleDelete = async (id) => {
    ...
    const ok = await patientAPI.delete(id);
    if (ok) {
  ...
  const filtered = patients.filter(p => {
    ...
    const matchSearch = !s || (p.patientCode||"").toLowerCase().includes(s)
      || (p.disease||"").toLowerCase().includes(s)
      || (p.region||"").toLowerCase().includes(s);
    const matchDisease = !diseaseFilter || p.disease === diseaseFilter;
    const matchRegion = !regionFilter || p.region === regionFilter;
    return matchSearch && matchDisease && matchRegion;
  });
  ...
  return (
    <div className="p-6 space-y-5">
      <PageHeader
        ...
        subtitle={`${total} patients in database`}
        action={
          <div className="flex gap-2">
            <SecondaryBtn onClick={loadPatients}>🔄 Refresh</SecondaryBtn>
            <PrimaryBtn onClick={() => { setEditPatient(null); setForm(EMPTY_FORM); setShowForm(true); }}>
              + New Patient
            </PrimaryBtn>
          </div>
        }
      ...
      <Card className="overflow-hidden">
        {loading ? <Spinner /> : filtered.length === 0 ? (
          <EmptyState icon="👤" title="No patients found" desc="Try changing your filters or add a new patient" />
        ) : (
          <>
                ...
                {filtered.map((p, i) => (
                  <tr key={p.id || i} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-slate-500 text-xs">
                      {(p.reportDate || p.report_date || "").toString().split("T")[0]}
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex gap-2">
                        <button onClick={() => handleEdit(p)} className="text-slate-400 hover:text-amber-500 transition-colors text-lg">✏</button>
                        <button onClick={() => handleDelete(p.id)} className="text-slate-400 hover:text-red-500 transition-colors text-lg">🗑</button>
                      </div>
                    </td>
                  </tr>
                ))}
